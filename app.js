...
function vehicleCard(v){const a=dateStatus('Ασφάλεια',v.insuranceDate),k=dateStatus('ΚΤΕΟ',v.kteoDate),s=serviceStatus(v);return`<article class="vehicle-card"><div class="vehicle-photo">${v.photo?`<img src="${v.photo}">`:emoji(v.type)}</div><div><h3>${esc(displayName(v))}</h3><div class="vehicle-meta">${emoji(v.type)} ${esc(v.type)}${v.plate?' • '+esc(v.plate):''}</div><div class="chips"><span class="chip ${a.cls}">${a.text}</span><span class="chip ${k.cls}">${k.text}</span><span class="chip ${s.cls}">${s.text}</span></div><div class="vehicle-actions"><button class="view" onclick="viewVehicle('${v.id}')">Προβολή</button><button class="edit" onclick="editVehicle('${v.id}')">Επεξεργασία</button><button class="pdf" onclick="exportVehiclePdf('${v.id}')">PDF</button><button class="delete" onclick="deleteVehicle('${v.id}')">Διαγραφή</button></div></div></article>`}
